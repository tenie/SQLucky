package net.tenie.plugin.sqlServerConnector.impl;

import net.tenie.Sqlucky.sdk.SqluckyPluginDelegate;
import net.tenie.Sqlucky.sdk.component.ComponentGetter;

public class SqlServerConnectorDelegateImpl implements SqluckyPluginDelegate {
	public static final String pluginName = "Sql Serve Connector";
	public static final String pluginCode = "net.tenie.plugin.SqlServerConnector";
	public static final String pluginDescribe = "Sql Server Connector";
	public static final String version ="0.0.1";
	@Override
	public void register() {
//		System.out.println("register:  MysqlConnectorDelegateImpl ..."); 
		// 注册
		SqlServerRegister reg = new SqlServerRegister();
		ComponentGetter.appComponent.registerDBConnector(reg);
		
		
	}
	@Override
	public void showed() {
		
	}

	@Override
	public void unload() {
//		System.out.println("unload: MysqlConnectorDelegateImpl...");

	}

	@Override
	public String pluginName() { 
		return pluginName;
	}
	@Override
	public void load() {
		
	}
	@Override
	public String pluginCode() {
		return pluginCode;
	}
	@Override
	public String pluginDescribe() {
		return pluginDescribe;
	}
	@Override
	public String version() {
		return version;
	}

}
