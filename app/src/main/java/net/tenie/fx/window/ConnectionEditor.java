...
public class ConnectionEditor {
    // 编辑连接时记录连接状态
    public boolean editLinkStatus = false;
    ...
    public ConnectionEditor(boolean editLinkStatus, TreeItem<TreeNodePo> treeItem) {
        ...
        this.editTreeItem = treeItem;
        this.updateConnectionInfoSetting();
    }
...
//					DBinfoTree.openConn(item, false);
                    if (editTreeItem != null) {
                        DBinfoTree.openConn(editTreeItem, false);
                    }
                });
            }
        });
    ...
    public void ConnectionInfoSetting(boolean isEdit, String connNameVal, String userVal, String passwordVal,
...
//				connectionName,
    ...
    private static void treeItemDown(TreeItem<TreeNodePo> val) {
        TreeItem<TreeNodePo> parent = val.getParent();
        List<TreeItem<TreeNodePo>> allItems = parent.getChildren();
        if (allItems.size() > 1) {
            // 先移除
            parent.getChildren().remove(val);
            // 找到
            for (int i = 0; i < allItems.size(); i++) {
                var tmp = allItems.get(i);
                if (tmp.getChildren().size() > 0) {
                    continue;
                } else {
                    parent.getChildren().add(i, val);
                    break;
                }
            }
        }
    ...
    public Button createSaveBtn(Function<String, SqluckyConnector> assembleSqlCon, TextField connectionName,
                                SqluckyConnector dp) {
        ...
        saveBtn.setOnMouseClicked(e -> {
            ...
            if (connpo != null) {
                ...
                if (dp != null) {
                    ...
                    this.editTreeItem.getValue().setName(connectionName.getText());
                    AppWindow.treeView.refresh();
                } else {
                    // 新增一个节点
