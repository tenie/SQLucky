 ...
 *
 */
public class MyAreaTab2 extends Tab implements SqluckyTab {
	private boolean isInit = false;
	private DocumentPo docPo;
	private SqluckyEditor sqlCodeArea;
	// 放查找面板, 文本area 的容器
	private VBox vbox;
	// 查找面板
	...
	public MyAreaTab2(boolean save) {
		super();
		this.savePo = save;
	}

	public MyAreaTab2(String TabName) {
		super();
...
//	}
	...
	public MyAreaTab2(DocumentPo po, boolean save) {
		super();
		this.savePo = save;
		if (save) {
			if (po.getId() == null) {
	...
	private void createMyTab() {
		...
		sqlCodeArea = new HighLightingEditor(myAuto, this);
//		右键菜单
		HighLightingEditorContextMenu cm = new HighLightingEditorContextMenu(sqlCodeArea);
...
//	}
	...
	public String getTabTitle() {
		String title = CommonUtils.tabText(this);
		return title;
	}
...
//				}
	...
	// 添加空文本的codeTab
	public static MyAreaTab2 addCodeEmptyTabMethod() {
		var myTabPane = ComponentGetter.mainTabPane;
		int size = myTabPane.getTabs().size();
	...
	// 添加空文本的codeTab
	public static MyAreaTab2 addMyTabByScriptPo(DocumentPo scpo) {
		var myTabPane = ComponentGetter.mainTabPane;
		int size = myTabPane.getTabs().size();
		ConfigVal.pageSize++;
		MyAreaTab2 nwTab = new MyAreaTab2(scpo, true);
		myTabPane.getTabs().add(size, nwTab);// 在指定位置添加Tab
		myTabPane.getSelectionModel().select(size);
		ScriptTabTree.treeRootAddItem(nwTab);
		return nwTab;
...
//	}
	...
	// TODO 添加空文本的codeTab
	public static void mainTabPaneAddAllMyTabs(List<MyAreaTab2> ls) {
...
//		syncScriptPo();
	...
	public void setSqlCodeArea(HighLightingEditor sqlCodeArea) {
