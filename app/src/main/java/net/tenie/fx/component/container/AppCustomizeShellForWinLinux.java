...
/**
 * 使用自定义的壳(最大化, 最小化, 关闭), 非全屏时使用系统的
 */
@Deprecated
public class AppCustomizeShellForWinLinux {
    private Scene scene;
    ...
    // 设置窗口的事件, 使用方式 setupWindow(primaryStage, AppWindow.appHeadContainer.getHeadHbox());
    ...
    private void setupSmallMaximizedProperty() {
        ...
        smallWindowStage.maximizedProperty().addListener((a, b, isMax) -> {
            ...
            if (isMax) {
                ...
                // 创建一个新的最大化的窗口
                app.initStage(smallWindowStage, this.scene);
                // 小窗口隐藏 , 直接销毁
                Platform.runLater(() -> {
