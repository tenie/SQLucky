...
//		mytb.getTableData().setSaveBtn(saveBtn);
//
...
//
//		mytb.getTableData().setDetailBtn(detailBtn);
//
...
//			CommonUtility.findTable(mytb.getTableData());
//		});
...
//			mytb.refreshData(ComponentGetter.currentDataTab().getTableData().isLock());
//		});
...
//	}
	...
	public static List<Node> DDLOptionBtns(SqluckyConnector sqluckyConn, MyBottomSheet mytb, String ddl,
		...
		if (table == null && false) {
			...
			saveBtn.setOnMouseClicked(e -> {
				// TODO 保存存储过程
				ComponentGetter.appComponent.runSQL(sqluckyConn, mytb.getSqlArea().getCodeArea().getText(), true);
