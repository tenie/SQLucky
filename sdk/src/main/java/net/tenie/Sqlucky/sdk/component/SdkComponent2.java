 ...
 *
 */
public class SdkComponent2 {

	private static Logger logger = LogManager.getLogger(SdkComponent2.class);
...
//			});
	...
	private static FilteredTableColumn<ObservableList<StringProperty>, String> createColumnForShowInfo(String colname,
		...
		FilteredTableColumn<ObservableList<StringProperty>, String> col = SdkComponent2.createColumn(colname, colIdx);
	...
	public static void sqlResultShow2(String sql, Connection conn, String tableName, List<Node> nodeList)
			throws Exception {
		try {
			FilteredTableView<ObservableList<StringProperty>> table = SdkComponent2.creatFilteredTableView();
			// 查询的 的语句可以被修改
			...
			// table 添加列和数据
			// 表格添加列
			var tableColumns = SdkComponent2.createTableColForInfo(colss);
			// 设置 列的 右键菜单
...
//				System.gc();
				MyOption.gc(SdkComponent2.class, "deleteEmptyTab");
			});
		}
	...
	public static void hideBottom() {
		...
		if(showStatus ) { 
			SdkComponent2.escapeWindowsUiBug(); 
		}
	}
	...
	public static void clearDataTable(Tab tb) {
		...
		long costTime = (endtime - begintime);
//		System.gc();
		MyOption.gc(SdkComponent2.class, "clearDataTable");
		logger.info("关闭使用时间 = "+ costTime);
