...
//			// 获取元数据
			...
			ObservableList<ResultSetRowPo> val;
			// 数据
			if (limit > 0) {
...
//			 val = simpleExecRs(rs, fields); 
	...
	private static ObservableList<ResultSetRowPo> execRs(int limit, ResultSet rs, ObservableList<SheetFieldPo> fpo,
...
//		ObservableList<ObservableList<StringProperty>> allDatas = FXCollections.observableArrayList();
		...
		ObservableList<ResultSetRowPo> allDatas = FXCollections.observableArrayList();
		while (rs.next()) {
...
//				String field = fpo.get(i).getColumnLabel().get();
				...
				addStringPropertyChangeListener(val, rn, i, rowDatas, dbtype);
	...
	public static ObservableList<ObservableList<StringProperty>> simpleExecRs(ResultSet rs,
			ObservableList<SheetFieldPo> fpo) throws SQLException {
		int idx = 1;
		int rowNo = 0;
		int columnnums = fpo.size();
		ObservableList<ObservableList<StringProperty>> allDatas = FXCollections.observableArrayList();
	...
	private static ObservableList<ResultSetRowPo> execRs(ResultSet rs, ObservableList<SheetFieldPo> fpo,
			SqluckyConnector dpo) throws SQLException {
	...
	public static void addStringPropertyChangeListener(StringProperty val, int rowNo, int idx,
			ObservableList<ResultSetCellPo> rowDatas, int dbtype) {
