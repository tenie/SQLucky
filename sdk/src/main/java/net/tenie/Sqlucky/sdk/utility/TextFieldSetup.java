...
public class TextFieldSetup {
	...
	// MaxRows限制只能输入数字
	public static void maxRowsNumberOnly(TextField tf) {
		tf.textProperty().addListener(MaxRowsTextFieldNumChange(tf));
	}
	...
	// 文本框只能输入数字
	public static ChangeListener<String> MaxRowsTextFieldNumChange(TextField rows) {
		return new ChangeListener<String>() {
			@Override
			public void changed(ObservableValue<? extends String> observable, String oldValue, String newValue) {
				if (StrUtils.isNotNullOrEmpty(newValue)) {
