...
public class MyTextField2TableCell3<ResultSetRowPo, T> extends TextFieldTableCell<ResultSetRowPo, T> {
    
    public static <ResultSetRowPo> Callback<TableColumn<ResultSetRowPo,String>, TableCell<ResultSetRowPo,String>> forTableColumn() {
        return forTableColumn(new DefaultStringConverter());
    }

    public static <ResultSetRowPo,T> Callback<TableColumn<ResultSetRowPo,T>, TableCell<ResultSetRowPo,T>> forTableColumn(
            final StringConverter<T> converter) {
        return list -> new MyTextField2TableCell3<>(converter);
    }


    private TextField textField;

    public MyTextField2TableCell3() {
        this(null);
    }

    public MyTextField2TableCell3(StringConverter<T> converter) {
        super(converter);
//        this.setMaxHeight(50);
            ...
            public void invalidated(Observable observable) {
                if (getGraphic() != null && getGraphic() instanceof TextField) {
                    ...
                    textField.addEventFilter(KeyEvent.KEY_PRESSED, event -> {
                        final TableView.TableViewSelectionModel<ResultSetRowPo> selectionModel = getTableView().getSelectionModel();
    ...
    @Override public void commitEdit(T item) {
        if (! isEditing() && ! item.equals(getItem())) {
            TableView<ResultSetRowPo> table = getTableView();
            if (table != null) {
                TableColumn<ResultSetRowPo, T> column = getTableColumn();
                TableColumn.CellEditEvent<ResultSetRowPo, T> event = new TableColumn.CellEditEvent<>(table,
